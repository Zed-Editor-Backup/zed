...
impl ResolvedPatch {
    pub fn apply(
        ...
        let multibuffer = editor.multibuffer().read(cx).snapshot(cx);
        let mut old_excerpts = multibuffer.excerpts().peekable();
        let mut new_excerpts = edit_groups
            .iter()
            .flat_map(|(buffer, groups)| {
                groups
                    .iter()
                    .map(move |group| (buffer, group.context_range.clone()))
            })
            .peekable();
        ...
        loop {
            ...
            let new_buffer = editor.branch_buffer_for_base(&new_buffer).unwrap();
