...
impl X11WindowState {
    ...
    pub fn new(
        ...
        // Construct the X11WindowHandle immediately after window creation so that it gets destroyed
        // if an error occurs below.
        let x_window = X11WindowHandle {
            id: x_window,
            xcb: xcb.clone(),
        };
        ...
        if let Some(size) = params.window_min_size {
            ...
            check_reply(
                ...
                size_hints.set_normal_hints(xcb, x_window.id),
            )?;
        }

        let reply = get_reply(|| "X11 GetGeometry failed.", xcb.get_geometry(x_window.id))?;
        if reply.x == 0 && reply.y == 0 {
            ...
            check_reply(
                ...
                xcb.configure_window(x_window.id, &xproto::ConfigureWindowAux::new().x(x).y(y)),
            )?;
        }
        ...
        let config = BladeSurfaceConfig {
            ...
            size: query_render_extent(xcb, x_window.id)?,
        ...
        check_reply(|| "X11 MapWindow failed.", xcb.map_window(x_window.id))?;
...
impl Drop for X11Window {
    fn drop(&mut self) {
        ...
        // Mark window as destroyed so that X11 events for it can be filtered out.
