use crate::{
    item::{
        ClosePosition, ClosingStrategy, Item, ItemHandle, ItemSettings, PreviewTabsSettings,
...
impl Pane {
    ...
    fn _remove_item(
        ...
        let closing_strategy = ItemSettings::get_global(cx).closing_strategy;
        self.activation_history
        ...
        if item_index == self.active_item_index {
            let index_to_activate = match closing_strategy {
                ClosingStrategy::History => self
                    .activation_history
                    .pop()
    ...
    async fn test_remove_item_ordering_neighbour(cx: &mut TestAppContext) {
        ...
        cx.update_global::<SettingsStore, ()>(|s, cx| {
            s.update_user_settings::<ItemSettings>(cx, |s| {
                s.activate_on_close = Some(ClosingStrategy::Neighbour);
            });
        });
        let fs = FakeFs::new(cx.executor());
